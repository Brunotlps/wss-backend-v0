...
class CourseFilter(django_filters.FilterSet):
    ...
    category = django_filters.NumberFilter(
        field_name="category", help_text="Filter by category ID (exact match)"
    )
    category__slug = django_filters.CharFilter(
        field_name="category__slug",
        lookup_expr="exact",
        help_text='Filter by category slug (exact match, e.g., "web-development")',
    )
    instructor = django_filters.NumberFilter(
        field_name="instructor", help_text="Filter by instructor user ID (exact match)"
    )
    instructor__username = django_filters.CharFilter(
        field_name="instructor__username",
        lookup_expr="icontains",
        help_text="Filter by instructor username (partial match, case-insensitive)",
    )
    difficulty = django_filters.ChoiceFilter(
        choices=Course.DifficultyLevel.choices,
        help_text="Filter by difficulty level (BEG, INT, or ADV)",
    )
    is_published = django_filters.BooleanFilter(
        field_name="is_published",
        help_text="Filter by publication status (true/false). Non-staff users always see only published courses.",
    )
    price_min = django_filters.NumberFilter(
        field_name="price",
        lookup_expr="gte",
        help_text="Filter courses with price greater than or equal to this value",
    )
    price_max = django_filters.NumberFilter(
        field_name="price",
        lookup_expr="lte",
        help_text="Filter courses with price less than or equal to this value",
    )
    is_free = django_filters.BooleanFilter(
        method="filter_is_free",
        help_text="Filter free courses (price == 0). Use ?is_free=true",
    )
