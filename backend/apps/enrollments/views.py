...
class EnrollmentViewSet(viewsets.ModelViewSet):
    ...
    queryset = Enrollment.objects.select_related("user", "course").prefetch_related(
        "lesson_progress"
    )
...
class LessonProgressViewSet(viewsets.ModelViewSet):
    ...
    queryset = LessonProgress.objects.select_related(
        "enrollment", "enrollment__user", "enrollment__course", "lesson"
    )
    ...
    def get_queryset(self):
        ...
        if user.is_instructor:
            return queryset.filter(
                Q(enrollment__user=user) | Q(enrollment__course__instructor=user)
            )
