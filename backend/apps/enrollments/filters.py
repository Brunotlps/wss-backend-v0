...
class EnrollmentFilter(django_filters.FilterSet):
    ...
    course = django_filters.NumberFilter(field_name='course', help_text='Filter by Course ID')
    course__slug = django_filters.CharFilter(field_name='course__slug', lookup_expr='exact', help_text='Filter by course slug')
    is_active = django_filters.BooleanFilter(field_name='is_active', help_text='Filter by enrollment status (active/inactive)')
    completed = django_filters.BooleanFilter(field_name='completed', help_text='Filter by course completion status')
    certificate_issued = django_filters.BooleanFilter(field_name='certificate_issued', help_text='Filter by certificate issued status')
    rating = django_filters.NumberFilter(field_name='rating', lookup_expr='exact', help_text='Filter by exact rating (1-5)')
    rating_min = django_filters.NumberFilter(field_name='rating', lookup_expr='gte', help_text='Filter by minimum rating')
    rating_max = django_filters.NumberFilter(field_name='rating', lookup_expr='lte', help_text='Filter by maximum rating')
    has_rating = django_filters.BooleanFilter(method='filter_has_rating', help_text='Filter by presence of rating')
    enrolled_after = django_filters.DateTimeFilter(field_name='enrolled_at', lookup_expr='gte', help_text='Filter by enrollment date (on or after)')
    enrolled_before = django_filters.DateTimeFilter(field_name='enrolled_at', lookup_expr='lte', help_text='Filter by enrollment date (on or before)')
    completed_after = django_filters.DateTimeFilter(field_name='completed_at', lookup_expr='gte', help_text='Filter by completion date (on or after)')
    completed_before = django_filters.DateTimeFilter(field_name='completed_at', lookup_expr='lte', help_text='Filter by completion date (on or before)')
    has_review = django_filters.BooleanFilter(method='filter_has_review', help_text='Filter by presence of review')
...
class LessonProgressFilter(django_filters.FilterSet):
    ...
    enrollment = django_filters.NumberFilter(field_name='enrollment', help_text='Filter by Enrollment ID')
    lesson = django_filters.NumberFilter(field_name='lesson', help_text='Filter by Lesson ID')
    lesson__course = django_filters.NumberFilter(field_name='lesson__course', help_text='Filter by Course ID (through lesson)')
    completed = django_filters.BooleanFilter(field_name='completed', help_text='Filter by lesson completion status')
    watched_duration_min = django_filters.NumberFilter(field_name='watched_duration', lookup_expr='gte', help_text='Filter by minimum watched duration (minutes)')
    watched_duration_max = django_filters.NumberFilter(field_name='watched_duration', lookup_expr='lte', help_text='Filter by maximum watched duration (minutes)')
    last_watched_after = django_filters.DateTimeFilter(field_name='last_watched_at', lookup_expr='gte', help_text='Filter by last watched date (on or after)')
    last_watched_before = django_filters.DateTimeFilter(field_name='last_watched_at', lookup_expr='lte', help_text='Filter by last watched date (on or before)')
