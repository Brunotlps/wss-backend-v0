...
class Enrollment(TimeStampedModel):
    ...
    enrolled_at = models.DateTimeField(
        _("enrolled at"), auto_now_add=True, help_text=_("When the enrollment occurred")
    )
    ...
    def get_next_lesson(self):
        ...
        """
        completed_lesson_ids = self.lesson_progress.filter(completed=True).values_list(
            "lesson_id", flat=True
        )

        return (
            self.course.lessons.exclude(id__in=completed_lesson_ids)
            .order_by("order")
            .first()
        )
...
class LessonProgress(TimeStampedModel):
    ...
    class Meta:
        ...
        unique_together = [
            ["enrollment", "lesson"]
        ]  # One progress per lesson per enrollment
        indexes = [
