...
class LessonFilter(django_filters.FilterSet):
    ...
    course = django_filters.NumberFilter(
        field_name="course", help_text="Filter by course ID (exact match)"
    )
    ...
    is_free_preview = django_filters.BooleanFilter(
        field_name="is_free_preview", help_text="Filter by preview status (true/false)"
    )

    # Order filters (range support)
    order = django_filters.NumberFilter(
        field_name="order", help_text="Filter by specific lesson order number"
    )
